			...
			<table class="table table-hover table-bordered" id="list">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<?php if($row['status'] == 0): ?>
								<span class="badge badge-primary">진행중/열림</span>
							<?php elseif($row['status'] == 1): ?>
								<span class="badge badge-Info">작업중</span>
							<?php elseif($row['status'] == 2): ?>
								<span class="badge badge-success">완료</span>
							<?php else: ?>
								<span class="badge badge-secondary">닫힘</span>
							<?php endif; ?>
						</td>
						<td class="text-center">
		                    ...
		                    <div class="dropdown-menu" style="">
		                      ...
		                      <a class="dropdown-item" href="./index.php?page=edit_ticket&id=<?php echo $row['id'] ?>">변경</a>
		                      <div class="dropdown-divider"></div>
		                      <a class="dropdown-item delete_ticket" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">삭제</a>
		                    </div>
						</td>
					</tr>	
				<?php endwhile; ?>
...
<script>
	...
	_conf("진짜로 이 티켓을 삭제하시겠습니까?","delete_ticket",[$(this).attr('data-id')])
	})
	})
	function delete_ticket($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("티켓이 성공적으로 삭제됨",'success')
					setTimeout(function(){
						location.reload()
					},1500)
